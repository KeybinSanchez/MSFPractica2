...
u = np.array(pd.read_excel('D:/MDSF P2/signal.xlsx', header=None))
...
fg1 = plt.figure()
plt.plot(t, Pp0, '-', linewidth=1.5, color='#2E8B57', label='Pp(t): Normotenso')
plt.plot(t, Pp1, '-', linewidth=1.5, color='#1E90FF', label='Pp(t): Hipotenso')
plt.plot(t, Pp2, '-', linewidth=1.5, color='#FF6347', label='Pp(t): Hipertenso')
...
mycolors = ['#6A5ACD', '#FF8C00', '#20B2AA']

fg2,axs = plt.subplots(2,1)
fg2.set_size_inches(w,h*2)

axs[0].plot(t, Pp0, '-', linewidth=1.5, color=mycolors[0], label='$Pp(t):Normotenso$')
axs[0].plot(t, Pp1, '-', linewidth=1.5, color=mycolors[1], label='$Pp(t):Hipotenso$')
...
axs[0].set_ylabel('$Pp_i(t)$ $[V]$')
axs[0].legend(bbox_to_anchor=(0.5,1.12),loc='lower center',ncol=3,frameon=False)
axs[0].set_title('Normotenso vs Hipotenso', pad=20)
axs[0].grid(True, linestyle='--', alpha=0.4)

axs[1].plot(t, Pp0, '-', linewidth=1.5, color=mycolors[0], label='$Pp(t):Normotenso$')
axs[1].plot(t, Pp2, '-', linewidth=1.5, color=mycolors[1], label='$Pp(t):Hipertenso$')
...
axs[1].set_ylabel('$Pp_i(t)$ $[V]$')
axs[1].legend(bbox_to_anchor=(0.5,1.12),loc='lower center',ncol=3,frameon=False)
axs[1].set_title('Normotenso vs Hipertenso', pad=20)
